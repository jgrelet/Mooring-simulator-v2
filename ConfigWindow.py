...
class ConfigWindow(QWidget):
    ...
    def __init__(self, appName, version):
        ...
        self.config = Path(path.expandvars('$APPDATA/' + appName)).with_suffix('.toml')
        if not path.isfile(self.config):
            print(f"Configuration file don't exist, create one from default config to {self.config}")
            self.saveDefaultConfig()
        self.__cfg = toml.load(self.config)
    ...
    def saveDefaultConfig(self):
        with open(self.config, 'w') as fid:
